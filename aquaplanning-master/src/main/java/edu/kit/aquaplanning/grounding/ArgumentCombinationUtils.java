		...
		for (Type argType : argTypes) {
			...
			for (Argument c : allConstants) {
				if (problem.isArgumentOfType(c, argType)) {
					...
					//if else statement added to exclude block 3 and 4 from goal, 
					if ( c.toString().contains("block4") || c.toString().contains("block2"))
					{
					...
					//eligibleArgumentsAtPos.add(c); //commented out the statement when we added the if-else structure
				}
			}
